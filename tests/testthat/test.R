...
test_that("phi estimation with k=50", {
  r_results_k = robust_estimateCDCC(rt, delta=.975, k=50)
  r_phi_ape_k = abs(r_results_k$phi - phi) / phi
  
  expect_true(max(r_phi_ape_k) < .5)
})

test_that("high dimension phi estimation with k=50", {
  r_results_hight_dim_k = high_dimension_robust_estimateCDCC(
    rt=rt, 
    delta=.975, 
    k=50)

  r_phi_ape_hight_dim_k = abs(r_results_hight_dim_k - phi) / phi
  
  expect_true(max(r_phi_ape_hight_dim_k) < .5)
})
